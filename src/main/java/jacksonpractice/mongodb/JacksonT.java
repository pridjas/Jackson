package jacksonpractice.mongodb;

import jacksonpractice.elasticserach.ElasticSearch;
import jacksonpractice.model.JSONData;
import com.fasterxml.jackson.databind.MappingIterator;
import com.fasterxml.jackson.dataformat.csv.CsvMapper;
import com.mongodb.DBObject;
import com.mongodb.util.JSON;
import org.json.JSONObject;


import java.io.File;
import java.io.IOException;

public class JacksonT {
    final String fileLocation = "/Users/priyadeepjaswal/Downloads/613P001.2014.csv";

    public boolean defineSchema(MongoDB mongoDB, ElasticSearch elasticSearch, String dbName, String dbCollection) {
        boolean isInserted = true;
        try {
            if (mongoDB == null) {
                isInserted = false;
            } else if (elasticSearch == null) {
                isInserted = false;
            } else if (dbName == null) {
                isInserted = false;
            } else {
                File csvFile = new File(fileLocation);
                MappingIterator<JSONData> jsonDataMappingIterator =
                        new CsvMapper().readerWithTypedSchemaFor(JSONData.class).readValues(csvFile);

                while (jsonDataMappingIterator.hasNext()) {
                    JSONData jsonData = jsonDataMappingIterator.next();

                    JSONObject json = new JSONObject(jsonData);
                    DBObject dbObject = (DBObject) JSON.parse(json.toString());

                    elasticSearch.insertDoc(json.toString(), jsonData.getLocalDateValue());

                    if (mongoDB.insertToDB(dbObject, dbName, dbCollection) == false) {
                        isInserted = false;
                        break;
                    }


                }
            }


        } catch (IOException e) {
            System.out.println(e.getMessage());
            System.out.println("Database and elastic error");
        }
        return isInserted;

    }
}



















